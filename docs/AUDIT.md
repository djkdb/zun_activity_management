...
# 부록 B — 방향 정정 (2026-09-17)

소유자가 **터미널을 쓰지 않는다**고 확인해 줬다. "CLI 는 웹앱 안에 있다"는 말은
웹앱 내부의 입력 인터페이스를 뜻한 것이었다. 이 사실이 설계를 바꾼다.

## B-1. 구멍 하나를 먼저 막았다

`zun export` 가 `public/calendar.ics` 를 **파일로** 만드는 구조였다. 소유자가
터미널을 안 쓰면 그 파일이 영원히 갱신되지 않고, **웹에서 아무리 일정을 넣어도
아이폰 캘린더는 멈춰 있게 된다.**

→ `/calendar.ics` 를 **Cloudflare Pages Function** 으로 바꿨다
(`functions/calendar.ics.js`). 요청마다 Supabase 를 읽어 즉석에서 만든다.
Pages Functions 는 무료 티어이고 Anthropic API 와 무관하므로 비용 제약을 건드리지 않는다.

- 정적 파일이 Function 보다 우선하므로 `public/calendar.ics` 는 **지웠다.**
- `fold()` 가 Node 의 `Buffer` 를 쓰고 있어 Workers 에서 죽었다. `TextEncoder` 로
  바꿔 브라우저·Node·Workers 공용으로 만들었다.
- DB 장애 시 500 을 주면 아이폰이 구독을 버린다. **200 + 유효한 빈 캘린더**로
  돌려주고 원인은 `X-Zun-Error` 헤더에 담는다.

## B-2. 역할 재정의

```
웹 대시보드   ← 소유자가 쓰는 유일한 화면. 입력·확인·체크 전부 여기서.
Pages Function ← /calendar.ics 를 DB 에서 즉석 생성. 아이폰이 구독.
터미널 CLI    ← 소유자는 안 쓴다. Claude Code 세션에서 AI 파싱을 할 때만 쓰인다.
                (.claude/skills/zun-ingest 가 원문을 파싱해 zun save 를 호출)
```

**CLI 를 지우지는 않는다.** 브라우저에서 `claude` 서브프로세스를 못 돌리므로,
가이드북 붙여넣기 같은 복잡한 입력은 여전히 AI 가 필요하고 그 경로가 CLI 다.
다만 **소유자가 직접 칠 일은 없다** — 대화창에 붙여넣으면 내가 대신 돌린다.

## B-3. 그래서 남은 우선순위

앞서 세운 3단계(`zun today`/`week`/`search`)는 **터미널 명령이므로 가치가 낮아졌다.**
같은 기능을 웹에서 이미 보고 있다. 대신:

1. **웹 규칙 파서 강화** — 이제 주 입력 경로다. 기간("9/14~9/20"), 반복
   ("매주 화요일"), 여러 활동 동시 언급을 못 읽는다.
2. **노션 읽기 연동** — 커리어 Todo·Assignment 의 마감을 자동으로 끌어온다.
   이건 Pages Function 으로 하면 터미널 없이 주기적으로 돌 수 있다.
3. **웹에서 수정·삭제** — 지금은 추가와 체크만 된다.
