...
앱 설치 없이 **기본 캘린더 앱에 읽기 전용 달력 한 칸**이 생긴다.

```bash
zun export                    # public/calendar.ics 생성
git add public/calendar.ics && git commit -m "calendar" && git push
```

Cloudflare Pages 가 배포하면, 아이폰에서:

**설정 → 앱 → 캘린더 → 계정 → 계정 추가 → 기타 → 구독 캘린더 추가**
→ `https://<프로젝트>.pages.dev/calendar.ics`

- 기존 개인 일정과 **같은 화면에 겹쳐 보인다.** 달력 색도 따로 지정된다.
- **알림이 실제로 온다** — 마감·제출은 D-7/D-3/D-1, 회의·행사는 1시간 전.
- 수업·근로·알바는 **매주 반복(RRULE)** 으로 들어간다. `--no-schedule` 로 뺄 수 있다.
- `UID` 가 안정적이라 다시 내보내도 **중복이 생기지 않는다.**
- `public/_headers` 가 `Content-Type: text/calendar` 를 지정한다.
  이게 없으면 아이폰이 구독을 거부한다.

**한계: 읽기 전용이다.** 아이폰에서 체크해도 zun 으로 돌아오지 않는다.
완료 체크는 `zun done` 이나 웹 대시보드에서 한다.
...
| 옵션 | |
|---|---|
| `--out <경로>` | 저장 위치 (기본 `public/calendar.ics`) |
| `--no-schedule` | 수업·근로·알바를 빼고 내보냄 |
| `--no-alarms` | 알림을 빼고 내보냄 |
...
- 브라우저에서는 `claude` 서브프로세스를 못 돌리므로 **규칙 기반**으로 읽는다.
  복잡한 문장(가이드북 붙여넣기, 주차별 표)은 터미널에서 `zun "…"` 를 쓰면
  AI 가 읽는다.
