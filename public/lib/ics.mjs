...
import { seoulYMD, DOWS, addDays, kstISO } from './dates.mjs';
...
export function fold(line) {
  const b = ENC.encode(line);                     // Buffer 대신 TextEncoder — 브라우저·Node·Workers 공용
  ...
  while (start < b.length) {
    ...
    parts.push((parts.length ? ' ' : '') + DEC.decode(b.subarray(start, end)));
    start = end;
  }
  return parts.join('\r\n');
}
